...
export default function ActivePrincipals() {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        const response = await fetch('/api/user_activity/active-principals');
        if (!response.ok) {
  ...
  return (
    <div className={styles.container}>
      ...
      {sessions && sessions.length > 0 ? (
          ...
          <table className={styles.queryTable}>
            ...
            <tbody>
              {sessions.map((session) => (
                <tr key={session.id}>
                  <td>{session.id}</td>
                  <td>{session.school_id}</td>
                  <td>{session.schools?.name || 'N/A'}</td>
                  <td>{session.schools?.principal_name || 'N/A'}</td>
                  <td>{session.schools?.email || 'N/A'}</td>
        ...
        <p>No active principals found.</p>
      )}
    </div>
  );
}
